...
__constant__ unsigned char mul2[256] = {
...
__constant__ unsigned char mul3[256] = {
...
__device__ void AES::KeyExpansionCore(unsigned char* in , unsigned char i){
...
__device__ void AES::KeyExpansion(unsigned char* inputKey, unsigned char* expandedKeys){
    for(int i = 0; i < 16; i++){
...
__device__  void AES::SubBytes(unsigned char* state){
    for(int i = 0; i < 16; i++){
        state[i] = s_box[state[i]];
    }
}
__device__  void AES::ShiftRows(unsigned char* state) {
...
__device__  void AES::MixColumns(unsigned char* state){
    unsigned char tmp[16];
...
__device__ void AES::AddRoundKey(unsigned char* state, unsigned char* roundKey){
    for(int i = 0; i < 16; i++){
        state[i] ^= roundKey[i];
    }
}
__device__ void AES::Encrypt(unsigned char* message, unsigned char* key){
...
__host__ unsigned char* readFile(){
    ...
    // Determine the file size by moving to the end and getting the position
    file.seekg(0, std::ios::end);
    std::streamsize size = file.tellg();
    file.seekg(0, std::ios::beg);

    // Allocate a char array of the appropriate size
    unsigned char* buffer = new unsigned char[size + 1];  // +1 for null terminator
    buffer[size] = '\0';  // Null-terminate the array for convenience
