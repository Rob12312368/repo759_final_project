...
void AES::SubBytes_Inv(unsigned char* state){
    for(int i = 0; i < 16; i++){
...
void AES::ShiftRows_Inv(unsigned char* state) {
    unsigned char tmp[16];
...
void AES::MixColumns_Inv(unsigned char* state) {
    unsigned char tmp[16];
...
void AES::Decrypt(unsigned char* message, unsigned char* key){
    unsigned char state[16];
    for(int i = 0; i < 16; i ++){
        state[i] = message[i];
    }
    ...
    for(int i = 0; i < rounds; i++){
        ...
        MixColumns_Inv(expandedKey + 160 - (16 * (i+1)));
